...
var drawActions = false; // Enable or Disable draw actions
...
// Init after Loaded
//  $(window).bind('load', doInit);

//  function doInit(){
// 	setTimeout(function() { init(); }, 1000);
//  }

$(window).bind('load', preInit);

function preInit(){
	var interval = setInterval(function(){
		var li_length = $('#bgImages li').length;
		if(li_length > 2){
			init();
	...
	if(getMediaType(mediaUrl)=='youtube' || getMediaType(mediaUrl)=='vimeo' || getMediaType(mediaUrl)=='jwplayer' || getMediaType(mediaUrl)=='flash'){
		if(getMediaType(mediaUrl)=='youtube')
		{
			...
			if(ytPlayerReady)
			{
				...
				// $('#bgImageWrapper').prepend($('<img src="assets/images/background/video-background.jpg"/>').addClass('new source').css('opacity','0'));
				$('#bgImageWrapper').prepend($('<img src="assets/images/background/video-background-'+imageValue+'.jpg"/>').addClass('new source').css('opacity','0'));
			}
...
// Inner Page ReLoaded Actions
//added by Tony
function pageReLoaded(){
	// console.log("reloaded");
	// console.log($('#content').length);
	if($('#content').length <= 0){
		$('#contentBoxScroll').hide();
		// console.log("hide");
		return;
	}
